...
public class Board {
	private final Square BLANK = new Square(Dest.BLANK);

	// no cross updated YET
	private final Square[][] board = {
			{ new Square(Dest.CA), new Square(Route.BLUE), new Square(Route.BLUE), new Square(Route.BLUE), new Square(Dest.MI) },
			{ new Square(Route.GREEN), BLANK, BLANK, BLANK, new Square(Route.GREY) },
			{ new Square(Route.GREEN), BLANK, BLANK, BLANK, new Square(Route.GREY) },
			{ new Square(Dest.PA), new Square(Route.BLACK), new Square(Route.BLACK), new Square(Route.BLACK), new Square(Dest.NY) }, };
	
	private static final String TOPGUIDE = "   01234567890\n   ___________\n";
 
	public boolean placeDest(CardDest cardDest,CardRoute cardRoute, int r1, int c1, int r2, int c2) {

		if (invalidRow(r1) || invalidRow(c1) || invalidCol(r2) || invalidCol(c2))
			return false;
		if (board[r1][c1].hasDest(cardDest.getDest()) && board[r2][c2].hasDest(cardDest.getDest())) {
	...
	public boolean placeRoute(CardRoute cardRoute, int r1, int c1, int r2, int c2) {
		...
		if(board[midrow][midcol].hasRoute(cardRoute.getRoute())) {
